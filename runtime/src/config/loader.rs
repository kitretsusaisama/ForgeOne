...
pub fn load_config_from_file(config_path: &str) -> Result<RuntimeConfig> {
    ...
    let config_str = fs::read_to_string(config_path)
        .map_err(|e| ForgeError::IoError(format!("Failed to read file: {}", e)))?;
...
pub fn load_config_from_env(prefix: &str) -> Result<RuntimeConfig> {
    ...
    let json = serde_json::to_string(&env_vars).map_err(|e| ForgeError::ParseError {
        format: "json".to_string(),
        error: e.to_string(),
    })?;
...
pub fn load_config(config_path: Option<&str>, env_prefix: Option<&str>) -> Result<RuntimeConfig> {
    let span = ExecutionSpan::new("load_config", common::identity::IdentityContext::system());
...
fn merge_configs(base: RuntimeConfig, override_config: RuntimeConfig) -> RuntimeConfig {
    ...
    merged.default_resource_limits = override_config.default_resource_limits.clone();
...
mod tests {
    ...
    fn test_load_config_from_file_json() {
        ...
        assert_eq!(config.default_resource_limits.cpu_millicores, 1000);
    }
...
default_resource_limits:
  ...
  network_bps: 2097152"#;
        ...
        assert_eq!(config.enable_metrics, true);
        assert_eq!(config.default_resource_limits.cpu_millicores, 2000);
    ...
    fn test_merge_configs() {
        ...
        let base = RuntimeConfig {
            ...
            default_resource_limits: ResourceLimits {
                ...
                network_bps: 1_048_576,
            },
            // Add other required fields for RuntimeConfig::default()
            ..RuntimeConfig::default()
        };
        ...
        assert_eq!(merged.default_resource_limits.cpu_millicores, 2000);
        assert_eq!(merged.default_resource_limits.memory_bytes, 209_715_200);
        assert_eq!(merged.default_resource_limits.disk_bytes, 2_147_483_648);
        assert_eq!(merged.default_resource_limits.network_bps, 2_097_152);
    }
}
