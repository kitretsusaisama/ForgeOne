...
pub fn init_redb(options: RedbOptions) -> Result<()> {
    ...
    if options.shard_count > MAX_SHARDS {
        return Err(ForgeError::ConfigError(
            format!("Shard count {} exceeds maximum of {}", options.shard_count, MAX_SHARDS)
        ));
    }
    ...
    REDB_MANAGER.set(manager_arc.clone()).map_err(|_| ForgeError::DatabaseConnectionError("Redb manager already initialized".to_string()))?;
...
fn init_system_database(manager: Arc<RedbManager>) -> Result<()> {
    ...
    if let Some(parent) = db_path.parent() {
        std::fs::create_dir_all(parent).map_err(|e| {
            ForgeError::IoError { message: format!("Failed to create database directory: {}", e), source: None }
        })?;
    }
...
fn init_log_shard(manager: &RedbManager, db_path: PathBuf, shard: usize) -> Result<()> {
    ...
    // Open the database
    let db = Database::create(&db_path).map_err(|e| {
        ForgeError::DatabaseConnectionError(format!("Failed to create log database shard {}: {}", shard, e))
    })?;
    ...
    let tables = [
        ...
        ("checkpoints", TableDefinition::<&str, &str>::new("checkpoints")),
    ];
...
fn init_blob_shard(manager: &RedbManager, db_path: PathBuf, shard: usize) -> Result<()> {
    ...
    // Open the database
    let db = Database::create(&db_path).map_err(|e| {
        ForgeError::DatabaseConnectionError(format!("Failed to create blob database shard {}: {}", shard, e))
    })?;
    
    // Define tables
    let tables = [
        ("blob_metadata", TableDefinition::<&str, &str>::new("blob_metadata")),
        ("blob_chunks", TableDefinition::<&str, &str>::new("blob_chunks")),
        ("blob_index", TableDefinition::<&str, &str>::new("blob_index")),
    ];
...
fn init_event_database(manager: Arc<RedbManager>) -> Result<()> {
    ...
    if manager.sharding_enabled {
        for shard in 0..manager.shard_count {
            let db_path = manager.base_dir.join(format!("events_shard_{}.redb", shard));
...
fn init_event_shard(manager: &RedbManager, db_path: PathBuf, shard: usize) -> Result<()> {
    ...
    // Open the database
    let db = Database::create(&db_path).map_err(|e| {
        ForgeError::DatabaseConnectionError(format!("Failed to create event database shard {}: {}", shard, e))
    })?;
    
    // Define tables
    let tables = [
        ("events", TableDefinition::<&str, &str>::new("events")),
        ("event_index", TableDefinition::<&str, &str>::new("event_index")),
...
impl RedbManager {
    ...
    pub fn encryption_key(&self) -> Result<&[u8]> {
        if !self.encryption_enabled {
            return Err(ForgeError::DatabaseEncryptionError("Encryption is not enabled".to_string()));
        }
    ...
    pub fn get_all_events<T: for<'de> serde::Deserialize<'de>>(&self, topic: &str) -> crate::error::Result<Vec<T>> {
        let db = self.get_event_database(topic)?;
        let read_txn = db.begin_read().map_err(|e| crate::error::ForgeError::DatabaseTransactionError(format!("Failed to begin read transaction: {}", e)))?;
        let events_table = read_txn.open_table(redb::TableDefinition::<&str, &str>::new("events"))
            .map_err(|e| crate::error::ForgeError::DatabaseQueryError(format!("Failed to open events table: {}", e)))?;
        let mut events = Vec::new();
        for entry in events_table.iter().map_err(|e| crate::error::ForgeError::DatabaseQueryError(format!("Failed to iterate events: {}", e)))? {
            let (_, value) = entry.map_err(|e| crate::error::ForgeError::DatabaseQueryError(format!("Failed to read event: {}", e)))?;
            // Decode, decrypt, decompress as needed (see get_event for logic)
            let data = base64::decode(value.value()).map_err(|e| crate::error::ForgeError::SerializationError(format!("Failed to decode base64: {}", e)))?;
    ...
    pub fn add_to_dedup_cache(&self, data: &[u8], reference: &str) -> Result<Hash> {
        if !self.deduplication_enabled {
            return Err(ForgeError::ConfigError("Deduplication is not enabled".to_string()));
        }
...
impl LogManager {
    ...
    pub fn write_log(&mut self, entry: LogEntry) -> Result<String> {
        ...
        {
            let mut logs_table = write_txn.open_table(TableDefinition::<&str, &str>::new("logs")).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to open logs table: {}", e))
            })?;
            
            logs_table.insert(entry.id.as_str(), final_data.as_str()).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to insert log entry: {}", e))
            })?;
        } // logs_table borrow ends here
        
        {
            let mut log_index_table = write_txn.open_table(TableDefinition::<&str, &str>::new("log_index")).map_err(|e| {
            ...
            log_index_table.insert(index_key.as_str(), entry.id.as_str()).map_err(|e| {
        ...
        if let Some(checkpoint) = &checkpoint_marker {
            let mut checkpoints_table = write_txn.open_table(TableDefinition::<&str, &str>::new("checkpoints")).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to open checkpoints table: {}", e))
            })?;
            
            checkpoints_table.insert(checkpoint.as_str(), entry.id.as_str()).map_err(|e| {
    ...
    /// Read logs by topic
    pub fn read_logs_by_topic(&self, topic: &str, start_time: Option<DateTime<Utc>>, end_time: Option<DateTime<Utc>>, limit: usize) -> Result<Vec<LogEntry>> {
        let manager = RedbManager::get_instance()?;
        ...
        if manager.sharding_enabled {
            for shard in 0..manager.shard_count {
                ...
                let entries = self.read_logs_from_db(&db_name, topic, start_time, end_time, limit)?;
    ...
    fn read_logs_from_db(&self, db_name: &str, topic: &str, start_time: Option<DateTime<Utc>>, end_time: Option<DateTime<Utc>>, limit: usize) -> Result<Vec<LogEntry>> {
        let manager = RedbManager::get_instance()?;
        ...
        let log_index_table = read_txn.open_table(TableDefinition::<&str, &str>::new("log_index")).map_err(|e| {
            ForgeError::DatabaseQueryError(format!("Failed to open log_index table: {}", e))
        })?;
        
        let logs_table = read_txn.open_table(TableDefinition::<&str, &str>::new("logs")).map_err(|e| {
            ForgeError::DatabaseQueryError(format!("Failed to open logs table: {}", e))
        })?;
        ...
        // Query the index
        for result in log_index_table.range(start_key.as_str()..end_key.as_str()).map_err(|e| {
                    ...
                    if let Some(stored_hash) = &entry.content_hash {
                        ...
                        if computed_hash != *stored_hash {
                            return Err(ForgeError::IntegrityBreach(
                                format!("Log entry {} has invalid checksum", entry.id)
                            ));
                        }
                    }
                }
    ...
    pub fn read_logs_from_checkpoint(&self, checkpoint: &str, limit: usize) -> Result<Vec<LogEntry>> {
        let manager = RedbManager::get_instance()?;
        ...
        if manager.sharding_enabled {
            for shard in 0..manager.shard_count {
                ...
                let checkpoints_table = read_txn.open_table(
        ...
        // Get the checkpoint entry
        let entry_data = logs_table.get(entry_id.as_str())
        .map_err(|e| {
            ForgeError::DatabaseQueryError(format!("Failed to get log entry: {}", e))
        })?.ok_or_else(|| {
        ...
        self.read_logs_by_topic(&checkpoint_entry.topic, Some(checkpoint_entry.timestamp), None, limit)
    }
}
...
impl BlobManager {
    ...
    /// Store a blob
    pub fn store_blob(&self, name: &str, content_type: &str, data: &[u8], metadata: HashMap<String, String>, identity: &IdentityContext) -> Result<String> {
        let manager = RedbManager::get_instance()?;
        ...
        {
            let mut metadata_table = write_txn.open_table(TableDefinition::<&str, &str>::new("blob_metadata")).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to open blob_metadata table: {}", e))
            })?;
            
            metadata_table.insert(blob_id.as_str(), metadata_json.as_str()).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to insert blob metadata: {}", e))
            })?;
        } // metadata_table borrow ends here
        
        {
            let mut chunks_table = write_txn.open_table(TableDefinition::<&str, &str>::new("blob_chunks")).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to open blob_chunks table: {}", e))
            })?;
            ...
            for (i, chunk) in data.chunks(chunk_size).enumerate() {
                ...
                let final_chunk = if manager.is_encryption_enabled() {
                    let nonce = [0u8; 12]; // TODO: Use a secure random nonce in production!
                    let encrypted = encrypt_aes_gcm(&compressed_chunk, manager.encryption_key()?, &nonce)?;
                    STANDARD.encode(&encrypted)
                } else {
                ...
                chunks_table.insert(chunk_key.as_str(), base64::encode(&chunk_data).as_str()).map_err(|e| {
                    ForgeError::DatabaseQueryError(format!("Failed to insert blob chunk: {}", e))
                })?;
            }
        } // chunks_table borrow ends here
        
        {
            let mut index_table = write_txn.open_table(TableDefinition::<&str, &str>::new("blob_index")).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to open blob_index table: {}", e))
            })?;
            
            // Index by name
            index_table.insert(format!("name:{}", name).as_str(), blob_id.as_str()).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to insert blob name index: {}", e))
            })?;
            
            // Index by checksum
            index_table.insert(format!("checksum:{}", checksum).as_str(), blob_id.as_str()).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to insert blob checksum index: {}", e))
            })?;
            
            // Index by content type
            index_table.insert(format!("content_type:{}", content_type).as_str(), blob_id.as_str()).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to insert blob content type index: {}", e))
            })?;
    ...
    pub fn get_blob(&self, blob_id: &str) -> Result<(BlobMetadata, Vec<u8>)> {
        ...
        {
            let metadata_table = read_txn.open_table(TableDefinition::<&str, &str>::new("blob_metadata")).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to open blob_metadata table: {}", e))
            })?;
            
            let metadata_json = metadata_table.get(blob_id).map_err(|e| {
            ...
            // Get chunks
            let chunks_table = read_txn.open_table(TableDefinition::<&str, &str>::new("blob_chunks")).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to open blob_chunks table: {}", e))
            })?;
            ...
            for i in 0..metadata.chunk_count {
                let chunk_key = format!("{}:{}", blob_id, i);
                let chunk_data = chunks_table.get(chunk_key.as_str()).map_err(|e| {
                    ForgeError::DatabaseQueryError(format!("Failed to get blob chunk {}: {}", i, e))
                })?.ok_or_else(|| {
                    ForgeError::DatabaseQueryError(format!("Blob chunk {}:{} not found", blob_id, i))
                })?;
                
                // Decode from base64
                let encoded_chunk = chunk_data.value();
                let chunk_bytes = base64::decode(encoded_chunk)
                    .map_err(|e| ForgeError::SerializationError(format!("Failed to decode base64: {}", e)))?;
            ...
            for chunk in chunks {
                ...
                if manager.is_checksum_verification_enabled() {
                    ...
                    if computed_checksum != chunk.checksum {
                        return Err(ForgeError::IntegrityBreach(
                            format!("Blob chunk {}:{} has invalid checksum", blob_id, chunk.chunk_index)
                        ));
                    }
                }
            ...
            if manager.is_checksum_verification_enabled() {
                ...
                if computed_checksum != metadata.checksum {
                    return Err(ForgeError::IntegrityBreach(
                        format!("Blob {} has invalid checksum", blob_id)
                    ));
                }
            }
    ...
    pub fn delete_blob(&self, blob_id: &str) -> Result<()> {
        ...
        
        {
            ...
            
            {
                ...
                for i in 0..metadata.chunk_count {
                    ...
                    chunks_table.remove(chunk_key.as_str()).map_err(|e| {
                        ForgeError::DatabaseQueryError(format!("Failed to delete blob chunk {}: {}", i, e))
                    })?;
                }
            } // chunks_table borrow ends here
            
            {
                ...
                // Remove name index
                index_table.remove(format!("name:{}", metadata.name).as_str()).map_err(|e| {
                    ForgeError::DatabaseQueryError(format!("Failed to delete blob name index: {}", e))
                })?;
                
                // Remove checksum index
                index_table.remove(format!("checksum:{}", metadata.checksum).as_str()).map_err(|e| {
                    ForgeError::DatabaseQueryError(format!("Failed to delete blob checksum index: {}", e))
                })?;
            } // index_table borrow ends here
    ...
    fn list_blobs_from_db(&self, db_name: &str, content_type: &str) -> Result<Vec<BlobMetadata>> {
        ...
        {
            let index_table = read_txn.open_table(TableDefinition::<&str, &str>::new("blob_index")).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to open blob_index table: {}", e))
            })?;
...
impl EventManager {
...

    {
        let mut index_table = write_txn
            ...
            .map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to open event_index table: {}", e))
            })?;
    ...
    pub fn query_events(&self, start_time: Option<DateTime<Utc>>, end_time: Option<DateTime<Utc>>, limit: usize) -> Result<Vec<String>> {
        let manager = RedbManager::get_instance()?;
    ...
    fn query_events_from_db(&self, db_name: &str, start_time: Option<DateTime<Utc>>, end_time: Option<DateTime<Utc>>, limit: usize) -> Result<Vec<String>> {
        let manager = RedbManager::get_instance()?;
        let db = manager.get_database(db_name)?;
        ...
        {
            let event_index_table = read_txn.open_table(TableDefinition::<&str, &str>::new("event_index")).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to open event_index table: {}", e))
            })?;
            ...
            // Query the index
            for result in event_index_table.range(start_key.as_str()..end_key.as_str()).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to query event index: {}", e))
            })? {
                let (_, event_id) = result.map_err(|e| {
                    ForgeError::DatabaseQueryError(format!("Failed to read event index entry: {}", e))
                })?;
    ...
    pub fn get_event(&self, event_id: &str) -> Result<String> {
        ...
        {
            let events_table = read_txn.open_table(TableDefinition::<&str, &str>::new("events")).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to open events table: {}", e))
            })?;
            
            let event_data = events_table.get(event_id).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to get event: {}", e))
            })?.ok_or_else(|| {
...
impl TopicLogManager {
    ...
    /// Write a log entry
    pub fn write_log(&mut self, level: LogLevel, message: &str, metadata: Option<HashMap<String, String>>) -> Result<String> {
        let manager = RedbManager::get_instance()?;
        self.check_rotation()?;
        let level_clone = level.clone();
        let entry_id = format!("{}_{}_{}", self.current_topic, Utc::now().timestamp_nanos(), Uuid::new_v4());
        ...
        })?;
        {
            ...
            logs_table.insert(entry_id.as_str(), final_data.as_str()).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to insert log entry: {}", e))
            })?;
        } // logs_table borrow ends here
        {
            ...
            let index_key = format!("{}:{:?}:{}", self.current_topic, level, Utc::now().timestamp_nanos());
            log_index_table.insert(index_key.as_str(), entry_id.as_str()).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to insert log index: {}", e))
            })?;
        } // log_index_table borrow ends here
        {
            let mut metadata_table = write_txn.open_table(TableDefinition::<&str, &str>::new("log_metadata")).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to open log_metadata table: {}", e))
            })?;
            ...
            let metadata_value = format!("{},{},{}", 
                ...
                self.current_log_size + final_data.len() as u64,
                Utc::now().timestamp());
                metadata_table.insert(metadata_key.as_str(), metadata_value.as_str()).map_err(|e| {
    ...
    fn check_rotation(&mut self) -> Result<bool> {
        let now = Utc::now();
        let time_since_rotation = now.signed_duration_since(self.last_rotation).num_seconds() as u64;
    ...
    fn rotate_log(&mut self) -> Result<()> {
        ...
        {
            let mut snapshots_table = write_txn.open_table(TableDefinition::<&str, &str>::new("log_snapshots")).map_err(|e| {
                ForgeError::DatabaseQueryError(format!("Failed to open log_snapshots table: {}", e))
            })?;
            ...
            let snapshot_metadata = format!("{},{},{},{}", 
                ...
                Utc::now().timestamp(),
                self.entry_count);
                snapshots_table.insert(snapshot_id.as_str(), snapshot_metadata.as_str()).map_err(|e| {
                    ForgeError::DatabaseQueryError(format!("Failed to insert snapshot metadata: {}", e))
                })?;
        } 
        write_txn.commit().map_err(|e| {
            ForgeError::DatabaseTransactionError(format!("Failed to commit transaction: {}", e))
    ...
    pub fn query_logs(&self, level: Option<LogLevel>, start_time: Option<DateTime<Utc>>, 
                      end_time: Option<DateTime<Utc>>, limit: usize) -> Result<Vec<LogEntry>> {
        let manager = RedbManager::get_instance()?;
        ...
        if manager.sharding_enabled {
            ...
            for shard in 0..manager.shard_count {
                let db_name = format!("logs_shard_{}", shard);
                let logs = self.query_logs_from_db(&db_name, level_clone.clone(), start_time, end_time, limit)?;
    ...
    fn query_logs_from_db(&self, db_name: &str, level: Option<LogLevel>, 
                          start_time: Option<DateTime<Utc>>, end_time: Option<DateTime<Utc>>, 
                          limit: usize) -> Result<Vec<LogEntry>> {
        let manager = RedbManager::get_instance()?;
        let db = manager.get_database(db_name)?;
        ...
        let start_key = if let Some(start) = start_time {
            format!("{}:{}:{}", self.current_topic, level_str, start.timestamp_nanos())
        } else {
            format!("{}:{}:", self.current_topic, level_str)
        };
        
        let end_key = if let Some(end) = end_time {
            format!("{}:{}:{}", self.current_topic, level_str, end.timestamp_nanos() + 1) // Add 1 to make it inclusive
        } else {
