...
pub fn get_snapshot_manager() -> Result<Arc<SnapshotManager>> {
    match SNAPSHOT_MANAGER.read().unwrap().as_ref() {
        ...
        None => Err(ForgeError::ConfigError("Snapshot manager not initialized".to_string())),
    }
}
...
fn load_snapshots(manager: &SnapshotManager) -> Result<()> {
    ...
    for entry in entries {
        let entry = entry.map_err(|e| ForgeError::IoError { message: e.to_string(), source: None })?;
        let path = entry.path();
        ...
        if path.is_file() && path.extension().map_or(false, |ext| ext == "json") {
            ...
            let metadata: SnapshotMetadata = serde_json::from_str(&metadata_json)
                .map_err(|e| ForgeError::DatabaseQueryError(format!("Failed to parse snapshot metadata: {}", e)))?;
    ...
    *manager.snapshots.write().unwrap() = snapshots;
    ...
    if schedule_path.exists() {
        ...
        let schedule: SnapshotSchedule = serde_json::from_str(&schedule_json)
            .map_err(|e| ForgeError::DatabaseQueryError(format!("Failed to parse snapshot schedule: {}", e)))?;
...
impl SnapshotManager {
    ...
    pub fn create_snapshot(
        ...
        // Add to map
        self.snapshots.write().unwrap().insert(snapshot_id, metadata.clone());
    ...
    pub fn restore_from_snapshot(
        ...
        let metadata = self.get_snapshot(snapshot_id)?
        .ok_or_else(|| ForgeError::DatabaseQueryError(format!("Snapshot {} not found", snapshot_id)))?;
        
        // Create restore snapshot before restoring
        self.create_snapshot(
            "pre-restore-backup",
            Some(&format!("Automatic backup before restoring snapshot {}", snapshot_id)),
            SnapshotType::Full,
            identity,
        ...
        self.create_snapshot(
            ...
            Some(&format!("Verification snapshot after restoring {}", snapshot_id)),
            SnapshotType::Full,
            identity,
    ...
    pub fn delete_snapshot(&self, snapshot_id: &str) -> Result<()> {
        ...
        let metadata_path = self.base_dir.join("metadata").join(format!("{}.json", snapshot_id));
    ...
    pub fn check_auto_snapshot(&self, identity: &IdentityContext) -> Result<bool> {
        ...
        let should_snapshot = match last_auto {
            Some(time) => {
                ...
                .map_err(|_| ForgeError::DatabaseError(crate::error::DatabaseErrorKind::DatabaseQueryError))?;
    ...
    fn apply_retention_policies(&self) -> Result<()> {
        ...
        for (tag, mut snapshots) in tag_groups {
            ...
            let min_to_keep = snapshots
                .iter()
                .filter_map(|(id, _)| {
                    self.snapshots.read().unwrap().get(id)
...
pub fn init_snapshot_system(options: &DbOptions) -> Result<()> {
    ...
    if let Err(e) = fs::create_dir_all(&snapshots_dir) {
        ...
        return Err(ForgeError::IoError { message: e.to_string(), source: None });
    }
